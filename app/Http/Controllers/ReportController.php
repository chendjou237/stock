...
class ReportController extends BaseController
{
    ...
    public function count_quantity_alert(request $request)
    {
     ...
     public function ProfitAndLoss(request $request)
     {
         ...
         // Calculate Daily Payments (Sales + Purchase Returns payments for today)
         $daily_payments = PaymentSale::where('deleted_at', '=', null)
             ->whereDate('date', Carbon::today())
             ->sum('montant');

         $daily_purchase_returns = PaymentPurchaseReturns::where('deleted_at', '=', null)
             ->whereDate('date', Carbon::today())
             ->sum('montant');

         $item['daily_payments'] = number_format($daily_payments + $daily_purchase_returns, 2, '.', ',');

         // Calculate Daily Expenses
         $daily_expenses = Expense::where('deleted_at', '=', null)
             ->whereDate('date', Carbon::today())
             ->sum('amount');

         $item['daily_expenses'] = number_format($daily_expenses, 2, '.', ',');

         // Calculate Daily Profit
         $daily_profit = ($daily_payments + $daily_purchase_returns) - $daily_expenses;
         $item['daily_profit'] = number_format($daily_profit, 2, '.', ',');
